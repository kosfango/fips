...
#if (_MSC_VER > 1000) && defined(HIDEVW)
#pragma once
#endif // _MSC_VER > 1000

#include "FListCtrl.h"		// в конце файла обявлен макрос HIDEVW
...
//	СТИЛИ		(функция GetFStyle/SetFStyle)
...
#define FL_PARENT_ROOT		309 // самый верхний родитель
#define FL_TOP				311	// верхняя видимая строка
#define FL_BOTTOM			312 // нижняя видимая строка
...
/**/	// операции с элементами строк
	...
	//	возвращает число
	int GetFItemNum(HFROW hFRow, int iFColumn);
	//	устанавливает текст 
	void SetFItemText(HFROW hFRow, int iFColumn, LPCSTR strText);	
	//	устанавливает число 
	void SetFItemNum(HFROW hFRow, int iFColumn, int num);	
...
protected:
	...
	HIDEVW*/ struct XFList_Color{	
		COLORREF crBackground;
		COLORREF crText;
	};

		//CONTROL PROFILE
	HIDEVW*/class XFList_ControlProfile{
	public:
		int m_nControlType;	
		...
		XFList_ControlProfile()
		{
			...
			m_ciSelect.strText.Empty();
			m_ciSelect.dwUserData=0;
		}
	};	
	HIDEVW*/ XFList_ControlProfile m_defaultControlProfile;
	HIDEVW*/ CArray<XFList_ControlProfile*, XFList_ControlProfile*> m_aControlProfiles;
	HIDEVW*/ CArray<XFList_ControlProfile*, XFList_ControlProfile*> *m_pRefControls;
	HIDEVW*/ CWnd* m_pInplaceControl;
	HIDEVW*/ HFROW m_hFRow;
	HIDEVW*/ int m_iFColumn;
	HIDEVW*/ UINT m_nChar;
	HIDEVW*/ XFList_ControlProfile* GetCP(int iControlProfile);		
	HIDEVW*/ XFList_ControlProfile* GetCP(HFROW hFRow, int iFColumn)
		{return GetCP(GetFItem(hFRow, iFColumn).m_iControlProfile);}	

protected:		//	IMPLEMENTATION
	
		//COLUMN
	HIDEVW*/ struct XFList_Column{	
	...
	HIDEVW*/ CArray<XFList_Column, XFList_Column> m_aFColumns;	
	HIDEVW*/ void InitFColumn(XFList_Column &col)
		{
	...
	HIDEVW*/ static int DefaultCompareFRows(CFListCtrl* pThis, int iFColumn, HFROW hFRow1, HFROW hFRow2)
		{
			return pThis->GetFItemText(hFRow1, iFColumn).CompareNoCase(
				pThis->GetFItemText(hFRow2, iFColumn));
		}
	HIDEVW*/ XFList_Column& GetFColumn(int iFColumn)
		{ 
...
protected:		//	IMPLEMENTATION
	...
	public:
		...
		HIDEVW*/ static void InitFItem(XFList_Item &item)
		{
			item.m_dwUserData=0;
		...
		HIDEVW*/ CArray<XFList_Item, XFList_Item> m_aFItems;		//жлементы строки

		HIDEVW*/ CList<HFROW, HFROW>	m_lstChildFRows;			//дочерние строки
		
		HIDEVW*/ BOOL m_bCollapsed;								//дочерняя ветвь закрыта
		HIDEVW*/ int	m_nDeep;									//уровень вложенности
		HIDEVW*/ int m_iListIndex;								//индекс в списке видимых строк

		HIDEVW*/ BOOL m_bBoldText;								//жирный текст
		XFList_Color m_color;

		HIDEVW*/ HFROW	m_hFRow;								//описатель в списке m_lstFRows
		HIDEVW*/ HFROW	m_hParentFRow;							//родительский описатель
		HIDEVW*/ POSITION m_posParentList;						//позиция в списке родителя
...
//	IMPLEMENTATION
	...
	friend XFList_Row;
	HIDEVW*/ CList<XFList_Row*, XFList_Row*> m_lstFRows;
	HIDEVW*/ XFList_Row m_rootFRow;
	HIDEVW*/ XFList_Row* GetFRow(HFROW hFRow)			
	{
...
protected:		//	IMPLEMENTATION
	...
	HIDEVW*/ void ShowFRow(HFROW hFRow, BOOL bShow=TRUE, BOOL bUpdateIndexes=TRUE);	
	HIDEVW*/ inline XFList_Row::XFList_Item& GetFItem(HFROW hFRow, int iFColumn)
		{			
		#ifdef _DEBUG
...
//
	...
	HIDEVW*/ XFList_Selection &GetSel()
		{return m_selection;}

protected:		//	IMPLEMENTATION
																	//IMPLEMENTATION
		//DRAW SERVICE	
	HIDEVW*/ CDC m_dcFItem;
	HIDEVW*/ CBitmap m_bmpFItem;
	HIDEVW*/ CSize m_szFItem;	
	HIDEVW*/ int	m_nTextLineHeight;
	HIDEVW*/ int GetFRowHeight(BOOL bReal=TRUE)
	{
	...
	HIDEVW*/ void UpdateMemDC(CDC *pDC, CRect rcFItem);
	HIDEVW*/ void DrawFItem(CDC* pDC, HFROW hFRow, int iFColumn);	
	HIDEVW*/ BOOL DrawElement(int nType, CDC *pDC, HFROW hFRow, int iFColumn, CRect *prcElement);		
	HIDEVW*/ void DrawItem(LPDRAWITEMSTRUCT lpDIS);		
																	//IMPLEMENTATION
		//OPTIONS
	HIDEVW*/ struct XFList_Options{
		int m_iExpandButton;  // [+]
	...
	HIDEVW*/ XFList_Options m_opt;	
	HIDEVW*/ const XFList_Options &GetOptions() {return m_opt;}	
...
#ifdef _DEBUG
	HIDEVW*/ virtual void AssertValid() const
	{		
...
protected:
	...
	HIDEVW*/ void TranslateMouse(UINT nFlags, CPoint point, BOOL bDoubleClick);	
	HIDEVW*/ BOOL HitTestMouseClick(CPoint point, HFROW &hFRow, int &iFColumn);

		// NOTIFY
	HIDEVW*/ void NotifyInit(FL_NOTIFY &notify, HFROW hFRow, int iFColumn, int code);
	HIDEVW*/ LRESULT NotifySend(FL_NOTIFY &notify);	

protected:	//	IMPLEMENTATION

		// EDIT 	
	HIDEVW*/struct FL_INPLACE_NOTIFY{
		NMHDR hdr;
		CString strText;
		DWORD	dwUserData;
		HFROW	hFRow;
	...
	HIDEVW*/ LRESULT InplaceControl(UINT code, UINT nChar, FL_INPLACE_NOTIFY* pInplaceNotify=NULL);
...
//	CFList_Edit
//
HIDEVW*/class CFList_Edit : public CEdit
{
	UINT m_nChar;
	HFROW m_hFRow;
	int m_iFColumn;
...
//		CFList_ComboBox
//
HIDEVW*/ class CFList_ComboBox : public CComboBox
{	
