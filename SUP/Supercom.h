...
#define BUFFERSIZE_RX   40960L   /* RX queue buffer len */
#define BUFFERSIZE_TX   12500L   /* TX queue buffer len */
#endif
...
int set_burst_mode(int state);
int append_char_to_burstcache(int ch);
int zmodem_compatibility_proc(int flag); // zum setzen von kompatibilitaeten


#include "bind.h"

int RS_TXaBlock(BYTE Com,char *cachebuf,int anzahl);
...
#define HAYES_INIT_STRING     "ATE0L1M0Q0V1&C1&D2&S1&W0"  /* modem init string */
...
#define acRESEND           9           /* Resending of a block */
#define acRESUME          10           /* Remote wants zm-resume */
...
#define acENDFILEXFER	 129           // !OLI!
#define acDELETEFILE	 130           // !OLI!
...
#define infSKIP             128  /* skip file (by oli) */
...
WORD COMMAPI ComGetSysTime(void);
WORD COMMAPI ComGetElapsedTime(WORD Start);
#else
DWORD COMMAPI ComBufSize(BYTE Com, TBufDir BufDir);
DWORD COMMAPI ComBufCount (BYTE Com, TBufDir BufDir);
DWORD COMMAPI ComGetSysTime(void);
DWORD COMMAPI ComGetElapsedTime(DWORD Start);
#endif
...
#define SEC0_005            5    /*   5 ms */
#define SEC0_01            10    /*  10 ms */
...
#if defined (_OS_NT_)
WORD COMMAPI RS_Global_Yield(HWND hWnd);
...
BOOLEAN COMMAPI RS_TXInTime(BYTE Com, char SCh, WORD WaitTime);
BOOLEAN COMMAPI RS_RXInTime(BYTE Com, LPCHAR RCh, WORD WaitTime);
...
void  COMMAPI   RS_Delay (WORD WaitTime);
...
void COMMAPI RS_TXCtlChar(BYTE Com, char CtlChar, WORD minWait, WORD maxWait);
...
void COMMAPI RS_TXPInTime(BYTE Com, LPCHAR Packet, WORD len, WORD OneChDelay, LPWORD  result);
void COMMAPI RS_RXPInTime(BYTE Com, LPCHAR Packet, WORD len, WORD OneChDelay, LPWORD  result);
...
BOOLEAN COMMAPI RS_Connect(BYTE Com, WORD ConnectDelay, BOOLEAN CheckModemResult, LPSTRING ModemResult, TINFOPROC InfoProc);
...
void COMMAPI RS_WaitLineClear(BYTE Com, TBufDir BufDir, WORD Timedelay);
...
BOOLEAN COMMAPI RS_TXFile_ZMODEM(BYTE Com, LPSTRING FileNameList, int far *ErrorCode, TINFOPROC InfoProc,int force1024);
BOOLEAN COMMAPI RS_RXFile_ZMODEM(BYTE Com,LPSTRING FileName, int far *ErrorCode, TINFOPROC InfoProc,char *basepath);
