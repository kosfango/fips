...
static char DlgName[]="IDD_MAIN_CONFIG";
...
bool CSAPrefsDialog::AddPage(CSAPrefsSubDlg &dlg,LPCSTR pCaption, CSAPrefsSubDlg* pDlgParent /*=NULL*/)
{
	if (m_hWnd)
	{
...
void CSAPrefsDialog::OnApply()
{
	if ((m_iCurPage >= 0) && (m_iCurPage < m_pages.GetSize()))
	{
		pageStruct *pPS = (pageStruct *)m_pages.GetAt(m_iCurPage);
		ASSERT(pPS);
		if (pPS && pPS->pDlg && ::IsWindow(pPS->pDlg->m_hWnd))
		{
			pPS->pDlg->m_bCloseOnOk=FALSE;
			pPS->pDlg->UpdateData(TRUE);
			pPS->pDlg->OnOK();
		}
	}
}
	...
	if ((m_iCurPage >= 0) && (m_iCurPage < m_pages.GetSize()))
	{
		...
		if (pPS)
		{
			if (pPS->pDlg)
			{
				if (::IsWindow(pPS->pDlg->m_hWnd))
				{
					pPS->pDlg->SendMessage(WM_HELP,0,0);
					// help!
/*					NMHDR nm;
					nm.code=PSN_HELP;WM_HELP
					nm.hwndFrom=m_hWnd;
