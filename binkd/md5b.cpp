...
/* ---------------------------------------------------------- */
void hmac_md5(unsigned char*  text,int text_len,unsigned char*  key,int key_len, MDcaddr_t digest)
{
        MD5_CTX context;
...
void getrand(char *res, int len, STATE *rnd)
{
  MDcaddr_t digest;
  struct {
    time_t tm;
  ...
  hmac_md5((unsigned char *)&rd, sizeof(rd), (unsigned char *)rnd, sizeof(STATE), digest);
  if((rnd->peer_name)&&(rnd->peer_name[0]))
    hmac_md5((unsigned char*)rnd->peer_name, strlen(rnd->peer_name), digest, sizeof(digest), digest);
