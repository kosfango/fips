...
int h_connect(int *so, struct sockaddr_in *name)
{
	...
	if(socks[0])
	{
		...
		else 
		{
			...
			if(!sp[0]) {
				buf[1]=1;
				send_socet(*so, buf, 3, 0);
			}
			else {
				buf[1]=2;
				if(sp1) buf[3]=2;
				else buf[3]=1;
				send_socet(*so, buf, 4, 0);
			}
			if((recv_socet(*so, buf, 2, 0)!=2)||((buf[1])&&(buf[1]!=1)&&(buf[1]!=2)))
			{
			...
			if(buf[1]==2) /* username/password method */
			{
				...
				send_socet(*so, buf, i, 0);
				buf[0]=buf[1]=0;
				if((recv_socet(*so, buf, 2, 0)<2)||(buf[1]))
				{
