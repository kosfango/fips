...
*/
	///////////////////////////////////////////////////////////////////////////////////
      ...
      {
				nCountFit++;
				cur_len = pdc->GetTextExtent(line, nCountFit).cx;
      }
      if (nCount > nCountFit)
				nCount = nCountFit;
			VERIFY (pdc->ExtTextOut(ptOrigin.x, ptOrigin.y, ETO_CLIPPED, &rcClip, line, nCount, NULL));
		}
		ptOrigin.x += pdc->GetTextExtent(line).cx;
	///////////////////////////////////////////////////////////////////////////////////
	}
}
	...
	LPCTSTR pszChars, int nOffset, int nCount, CPoint ptTextPos)
{
		...
		pBuf[nActualItem + 1].m_nCharPos <= nOffset + nCount)
	{
		...
		for (I = nActualItem; I < nBlocks - 1 && pBuf[I + 1].m_nCharPos <= nOffset + nCount; I ++)
		{
...
void CTextWnd::UpdateCaret()
{
  ...
  CSize sz = pdc->GetTextExtent(GetLineChars(m_ptCursorPos.y), m_ptCursorPos.x);
...
CFont *CTextWnd::GetFont (BOOL bItalic /*= FALSE*/ , BOOL bBold /*= FALSE*/)
{
	int nIndex = 0;
	if (bBold)	nIndex |= 1;
	if (bItalic)nIndex |= 2;
...
//	return GetScreenChars();
		...
		else
		{
			...
			int n = 0;
			CDC *pdc = GetDC();
			CFont *pOldFont = pdc->SelectObject(GetFont(FALSE, FALSE));
    
			CString eline;
			ExpandChars(pszChars, 0, GetLineLength(nLine), eline);
			while (n < eline.GetLength() && pdc->GetTextExtent(eline, n).cx < rect.Width()) n++;
...
int CTextWnd::GetScreenChars()
{
  if (m_nScreenChars == -1)
    ...
    //m_nScreenChars = rect.Width() / GetCharWidth();
		return rect.Width() / GetCharWidth();
	}
  return m_nScreenChars;
}
...
CPoint CTextWnd::ClientToText (const CPoint & point)
{
  ...
  CDC *pdc = GetDC();
	...
	if (pt.y >= 0 && pt.y < nLineCount)
	{
		...
		else
		{
			...
			{
				nOffsetChar++;
				cur_len = pdc->GetTextExtent(pszLine, nOffsetChar).cx;
			}
		}
  ...
  while (cur_len < point.x && nOffsetChar + nPos < nLength)
  {
		nPos++;		
		CString eline;
		ExpandChars(pszLine, nOffsetChar, nPos, eline);
		cur_len = pdc->GetTextExtent(eline).cx;
  }
  pdc->SelectObject(pOldFont);
	ReleaseDC(pdc);
	nPos += nOffsetChar;
...
CPoint CTextWnd::TextToClient (const CPoint & point)
{
  ...
  CFont *pOldFont = pdc->SelectObject(GetFont());
	...
	pt.x = pdc->GetTextExtent(eline).cx - m_nOffset;
  pdc->SelectObject(pOldFont);
  ReleaseDC(pdc);
	///////////////////////////////////////////////////////////////////////////////////
...
*/
  ...
  CSize sz_prev = pdc->GetTextExtent(pszLine + n, pt.x - n);
  CSize sz = pdc->GetTextExtent(pszLine + pt.x, 1);
  ...
  CRect rect;
  GetClientRect(&rect);
  if (sz_prev.cx + sz.cx > m_nOffset + rect.Width())
		nNewOffset = sz_prev.cx + sz.cx - rect.Width();
	if (sz_prev.cx < m_nOffset)
		nNewOffset = sz_prev.cx;
  if (nNewOffset < 0)
    nNewOffset = 0;
  if (m_nOffset != nNewOffset)
	{
	...
	DWORD dwFlags, int nLineIndex /*= -1*/)
{
  ...
  if ((dwFlags & UPDATE_SINGLELINE) != 0)
  {
    ...
    //  This line's actual length must be recalculated
    if (m_pnActualLineLength != NULL)
    {
...
CPoint CTextWnd::WordToRight (CPoint pt)
{
  ...
  while (pt.x < nLength)
	{
		if (isspace((unsigned char)pszChars[pt.x]))
			break;
    pt.x++;
	}
  ASSERT_VALIDTEXTPOS (pt);
...
CPoint CTextWnd::WordToLeft (CPoint pt)
{
  ...
  while (pt.x > 0)
  {
		if (isspace((unsigned char)pszChars[pt.x - 1]))
			break;
		pt.x--;
	}
  ASSERT_VALIDTEXTPOS (pt);
...
void CTextWnd::MoveWordLeft (BOOL bSelect)
{
  ...
  while (nPos > 0 && isspace((unsigned char)pszChars[nPos - 1]))
    nPos--;

  if (nPos > 0)
  {
		nPos--;
    if (!isspace((unsigned char)pszChars[nPos]))
			while (nPos > 0 && !isspace((unsigned char)pszChars[nPos - 1]))
				nPos--;
    else while (nPos > 0 && isspace((unsigned char)pszChars[nPos - 1])
			&& !isspace ((unsigned char)pszChars[nPos - 1]))
			nPos--;
	}
...
void CTextWnd::MoveWordRight(BOOL bSelect)
{
  ...
  if (!isspace((unsigned char)pszChars[nPos]))
    {
      while (nPos < nLength && !isspace((unsigned char)pszChars[nPos]))
        nPos++;
    }
  else
    {
      while (nPos < nLength && isspace((unsigned char)pszChars[nPos])
            && !isspace ((unsigned char)pszChars[nPos]))
        nPos++;
    }
...
void CTextWnd::MoveHome(BOOL bSelect)
{
  ...
  while (nHomePos < nLength && isspace((unsigned char)pszChars[nHomePos]))
    nHomePos++;
...
{ 
	if (m_bColorMark || nColorIndex < COLOR_EDIT_MARK0 || nColorIndex > COLOR_EDIT_MARK15) return FALSE;
