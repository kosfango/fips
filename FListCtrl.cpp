...
//  если hFRow==0, то nfrCode д.б. FL_TOP или FL_BOTTOM
...
//	поиск строки
//  если hParent==NULL, поиск идет по всему дереву
//  если hParent!=NULL и hFirst==NULL, поиск идет среди дочерних
...
HFROW hr,hp;
	...
	if (hParent) 
	{
		hp= hFirst ? hFirst : GetNextFRow(hParent, FL_FIRST_CHILD);
		if(bCompareNoCase)
		{
			for(hr=hp;hr;hr=GetNextFRow(hr))
				if(!text.CompareNoCase(GetFItemText(hr,iFColumn)))
					goto exit;
		}
		else
		{
			for(hr=hp;hr;hr=GetNextFRow(hr))
				if(GetFItemText(hr,iFColumn)==text)
					goto exit;
		}
	}
	else
	{
		if(bCompareNoCase)
		{
			for(hp=FindIndex(FL_ROOT,0);hp;hp=GetNextFRow(hp))
				for(hr=GetNextFRow(hp,FL_FIRST_CHILD);hr;hr=GetNextFRow(hr))
					if(!text.CompareNoCase(GetFItemText(hr,iFColumn)))
						goto exit;
		}
		else
...
//	возвращает число
int CFListCtrl::GetFItemNum(HFROW hFRow, int iFColumn) 
{	
	return atoi(GetFItem(hFRow, iFColumn).m_strText);
}

//	устанавливает текст 	
void CFListCtrl::SetFItemText(HFROW hFRow, int iFColumn, LPCSTR strText) 
{				
...
//	устанавливает число	
void CFListCtrl::SetFItemNum(HFROW hFRow, int iFColumn, int num) 
{
char buf[20];
	GetFItem(hFRow, iFColumn).m_strText=itoa(num,buf,10);	
	UpdateFColumnWidth(-1);
	UpdateFRow(hFRow);
}
...
COLORREF CFListCtrl::GetColor(int nType, HFROW hFRow, int iFColumn)
{
	...
	if(hFRow && hFRow==GetSelectedFRow())
	{
