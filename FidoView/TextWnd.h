...
enum
{
	COLOR_EDIT_NOTUSED,//для совместимости
...
private:
	...
	//Края (левое - пикселей, правое - символов)
	...
	int TextWidth(CDC *pdc, LPCSTR text, int len = -1)
	{
		return (m_lfBaseFont.lfPitchAndFamily & FIXED_PITCH) ?	(len >= 0 ? len : (int)strlen(text)) * GetCharWidth() : 
			(len >= 0 ? pdc->GetTextExtent(text, len).cx : pdc->GetTextExtent(text).cx);
	}
public:
...
protected:
	...
	CFont *GetFont (BOOL bItalic, BOOL bBold, BOOL bUnderline);
	...
	virtual BOOL GetUnderline(int nColorIndex);
...
public:
	...
	void SetLeftMargin(int m) { m_nLeftMargin = m; Invalidate(); }
	void SetRigthMargin(int m){ m_nRightMargin = m; Invalidate(); }
