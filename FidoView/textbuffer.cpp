...
int CTextBuffer::GetUndoDescription (string & desc, int pos /*= 0L*/ )
{
  ...
  //  Read description
	...
	desc = LangString(STR_UNDODESCRBEGIN + m_aUndoBuf[nPosition].m_nAction);
...
void CTextParser::WrapLine(int nLineIndex, int nMaxLineWidth, int *anBreaks, int &nBreaks)
{
	...
	int			nCharCount = 0;
	const char *szLine = m_pTextWnd->GetLineChars(nLineIndex);
	int			nLastBreakPos = 0;
...
unsigned int CFidoParser::ParseLine(unsigned int dwCookie, int nLineIndex, CTextBlock *pBuf, int &nActualItems)
{
	...
	if (strnicmp(ptr, "...", 3) == 0)
	{
		...
		*/
		int fake;
		unsigned nextCookie = (nLineIndex + 1) < m_pTextWnd->GetLineCount() ? ParseLine(-1U, nLineIndex + 1, 0, fake) : 
			COLOR_EDIT_ORIGIN;
